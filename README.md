# AnimeLoader

<div align="center">

![Python](https://img.shields.io/badge/Python-3.12+-blue.svg)
![License](https://img.shields.io/badge/License-MIT-green.svg)
![Status](https://img.shields.io/badge/Status-Active-success.svg)

**一个功能强大的动画订阅和下载管理系统**

[功能特性](#-功能特性) • [快速开始](#-快速开始) • [文档](#-文档) • [贡献指南](#-贡献指南)

</div>

---

## 📖 项目简介

AnimeLoader 是一个基于 Python 的动画订阅和下载管理系统，采用客户端-服务端架构设计。它可以帮助动画爱好者自动化地订阅、下载和管理动画内容，支持从多个 RSS 源自动获取新发布的动画，并提供智能解析功能来简化动画订阅流程。

### 核心特性

- 🎬 **智能动画订阅**：支持从蜜柑计划等动画网站智能解析动画信息
- 📡 **RSS源管理**：灵活管理多个 RSS 订阅源，支持自动检测新发布
- 🔗 **链接管理**：支持多种链接类型（magnet、ed2k 等）
- ⬇️ **多下载器支持**：支持 aria2、pikpak 等多种下载器
- ⏰ **定时任务调度**：自动检测新发布并下载
- 📊 **下载状态监控**：实时查看下载进度和状态
- 🖥️ **RESTful API**：提供完整的 REST API 接口
- 🧪 **完善测试**：包含完整的单元测试和集成测试

---

## ✨ 当前实现状态

### 已完成 ✅

- [x] 项目基础架构搭建
- [x] 数据库模型设计（SQLite + SQLAlchemy）
- [x] 动画管理服务（CRUD、搜索、过滤）
- [x] RSS 源管理服务（CRUD、关联动画）
- [x] 智能解析服务（蜜柑计划网站）
- [x] 链接管理服务（CRUD、类型过滤）
- [x] 下载器管理服务（Mock 实现）
- [x] 下载任务管理服务（Mock 实现）
- [x] 调度服务（定时任务管理）
- [x] 基础 API 框架（FastAPI）
- [x] 模块化路由和 Schemas 设计
- [x] 完整的测试套件（8 个测试模块）
- [x] 客户端基础框架

### 开发中 🚧

- [ ] RSS 源自动检查
- [ ] 链接自动下载
- [ ] 下载状态同步
- [ ] 命令行交互界面实现

### 计划中 📋

- [ ] 多下载器真实实现
- [ ] Web 界面
- [ ] 用户认证和权限管理
- [ ] 移动端支持

---

## 🏗️ 系统架构

```
┌─────────────────┐         ┌─────────────────┐
│                 │         │                 │
│   CLI Client    │◄────────┤   Server        │
│                 │  HTTP   │                 │
└─────────────────┘         └─────────────────�
                                     │
                                     │
                                     ▼
                              ┌─────────────────┐
                              │   SQLite DB     │
                              └─────────────────�
                                     │
                                     │
                                     ▼
                              ┌─────────────────┐
                              │   Downloaders   │
                              │  ┌───────────┐  │
                              │  │   aria2   │  │
                              │  └───────────┘  │
                              │  ┌───────────┐  │
                              │  │   pikpak  │  │
                              │  └───────────┘  │
                              └─────────────────┘
```

### 技术栈

**后端：**
- Python 3.12+
- FastAPI（REST API 框架）
- SQLAlchemy（ORM）
- SQLite（数据库）
- APScheduler（任务调度）
- Pydantic（数据验证）

**前端/客户端：**
- cmd2（命令行框架）
- rich（终端美化）
- requests（HTTP 客户端）
- feedparser（RSS 解析）
- beautifulsoup4（HTML 解析）

**其他：**
- pytest（测试框架）
- uvicorn（ASGI 服务器）

---

## 📁 项目结构

```
animeloader/
├── README.md              # 项目说明文档
├── LICENSE               # 许可证
├── requirements.txt      # 依赖列表
├── run_tests.py          # 测试运行脚本
├── server_config.yaml    # 服务端配置（开发环境）
├── client_config.yaml    # 客户端配置（开发环境）
├── docs/                 # 文档目录
│   ├── DESIGN.md         # 设计文档
│   └── TESTS.md          # 测试文档
├── server/               # 服务端代码
│   ├── __init__.py
│   ├── main.py           # 服务端入口
│   ├── models/           # 数据模型
│   │   ├── anime.py      # 动画模型
│   │   ├── rss_source.py # RSS源模型
│   │   ├── link.py       # 链接模型
│   │   ├── downloader.py # 下载器配置模型
│   │   └── download.py   # 下载任务模型
│   ├── services/         # 业务逻辑
│   │   ├── anime_service.py       # 动画服务
│   │   ├── rss_service.py         # RSS源服务
│   │   ├── smart_parser_service.py # 智能解析服务
│   │   ├── link_service.py        # 链接服务
│   │   ├── downloader_service.py  # 下载器服务
│   │   ├── download_service.py    # 下载服务
│   │   └── scheduler_service.py   # 调度服务
│   ├── link_parsers/     # 链接解析器
│   │   ├── base_parser.py      # 基础解析器
│   │   ├── magnet_parser.py    # 磁力链接解析器
│   │   └── ed2k_parser.py      # ed2k链接解析器
│   ├── site_parsers/      # 网站智能解析器
│   │   ├── base_site_parser.py # 基础网站解析器
│   │   └── mikan_parser.py     # 蜜柑计划解析器
│   ├── downloaders/       # 下载器实现
│   │   └── base_downloader.py   # 基础下载器接口
│   ├── api/              # API 接口（模块化设计）
│   │   ├── __init__.py
│   │   ├── routes/       # API 路由（按模块拆分）
│   │   │   ├── anime.py          # 动画相关路由
│   │   │   ├── anime_extra.py    # 动画扩展路由
│   │   │   ├── rss.py            # RSS源路由
│   │   │   ├── rss_extra.py      # RSS源扩展路由
│   │   │   ├── link.py           # 链接路由
│   │   │   ├── link_extra.py     # 链接扩展路由
│   │   │   ├── downloader.py     # 下载器路由
│   │   │   ├── download.py       # 下载任务路由
│   │   │   ├── scheduler.py      # 调度服务路由
│   │   │   ├── smart_parser.py   # 智能解析路由
│   │   │   └── health.py         # 健康检查路由
│   │   └── schemas/      # Pydantic 模型（按模块拆分）
│   │       ├── __init__.py
│   │       ├── common.py         # 通用响应模型
│   │       ├── anime.py          # 动画相关模型
│   │       ├── rss.py            # RSS源相关模型
│   │       ├── link.py           # 链接相关模型
│   │       ├── downloader.py     # 下载器相关模型
│   │       ├── download.py       # 下载任务相关模型
│   │       ├── scheduler.py      # 调度服务相关模型
│   │       └── smart_parser.py   # 智能解析相关模型
│   ├── database/         # 数据库相关
│   │   └── session.py    # 数据库会话
│   ├── tests/            # 服务端测试
│   │   ├── test_smart_parser.py   # 蜜柑计划解析器测试
│   │   ├── test_smart_add.py      # 智能添加功能测试
│   │   ├── test_link_service.py   # 链接服务测试
│   │   ├── test_downloader_service.py # 下载器服务测试
│   │   ├── test_download_service.py   # 下载服务测试
│   │   ├── test_scheduler_service.py  # 调度服务测试
│   │   └── test_api.py            # 服务端API测试
│   └── utils/            # 工具函数
│       ├── config.py     # 配置加载
│       └── logger.py     # 日志配置
├── client/               # 客户端代码
│   ├── __init__.py
│   ├── main.py           # 客户端入口
│   ├── commands/         # CLI 命令
│   │   └── anime_commands.py     # 动画命令
│   ├── api/              # API 客户端
│   │   └── client.py     # API客户端实现
│   ├── tests/            # 客户端测试
│   │   └── test_smart_add.py     # 客户端智能添加测试
│   └── utils/            # 工具函数
│       └── config.py     # 客户端配置加载
├── data/                 # 数据库文件目录
│   └── animeloader.db
├── downloads/            # 下载目录
└── logs/                 # 日志目录
```

---

## 🚀 快速开始

### 环境要求

- Python 3.12 或更高版本
- pip（Python 包管理器）

### 安装步骤

1. **克隆仓库**
```bash
git clone https://github.com/insomniacdoll/animeloader.git
cd animeloader
```

2. **创建虚拟环境**
```bash
python3 -m venv venv
source venv/bin/activate  # Linux/macOS
# 或
venv\Scripts\activate  # Windows
```

3. **安装依赖**
```bash
pip install -r requirements.txt
```

4. **初始化数据库**
```bash
python -m server.database.init_db
```

### 启动服务端

```bash
# 使用默认配置
python -m server.main

# 或指定配置文件
python -m server.main --config /path/to/config.yaml
```

服务端将在 `http://127.0.0.1:8000` 启动

API 文档访问地址：
- Swagger UI: http://127.0.0.1:8000/docs
- ReDoc: http://127.0.0.1:8000/redoc

### 启动客户端

```bash
# 使用默认配置
python -m client.main

# 或指定配置文件
python -m client.main --config /path/to/config.yaml
```

---

## 🧪 测试

### 运行所有测试

```bash
python run_tests.py
```

### 单独运行测试

```bash
# 服务端测试
python server/tests/test_smart_parser.py
python server/tests/test_smart_add.py
python server/tests/test_link_service.py
python server/tests/test_downloader_service.py
python/tests/test_download_service.py
python server/tests/test_scheduler_service.py
python server/tests/test_api.py

# 客户端测试
python client/tests/test_smart_add.py
```

### 测试覆盖

当前测试覆盖以下功能模块：

- ✅ 蜜柑计划解析器测试
- ✅ 智能添加功能测试
- ✅ 链接服务测试
- ✅ 下载器服务测试
- ✅ 下载服务测试
- ✅ 调度服务测试
- ✅ 服务端 API 测试
- ✅ 客户端智能添加测试

### 测试结果

所有测试应通过，输出如下：

```
============================================================
测试结果汇总
============================================================
1. 蜜柑计划解析器测试: ✓ 通过
2. 智能添加测试: ✓ 通过
3. 链接服务测试: ✓ 通过
4. 下载器服务测试: ✓ 通过
5. 下载服务测试: ✓ 通过
6. 调度服务测试: ✓ 通过
7. 服务端API测试: ✓ 通过
8. 客户端测试: ✓ 通过

============================================================
[成功] 所有测试通过
============================================================
```

---

## 📚 文档

- [设计文档](docs/DESIGN.md) - 详细的系统设计文档，包括架构设计、数据模型、API 接口等
- [测试文档](docs/TESTS.md) - 测试说明和运行指南

---

## ⚙️ 配置

### 服务端配置

服务端配置文件 `server_config.yaml` 示例：

```yaml
server:
  host: "127.0.0.1"
  port: 8000
  debug: false

database:
  path: "~/.animeloader/data/animeloader.db"

logging:
  level: "INFO"
  file: "~/.animeloader/logs/animeloader.log"

scheduler:
  enabled: true

smart_parser:
  timeout: 30
  user_agent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
  max_results: 10
  auto_add_rss: true
```

### 客户端配置

客户端配置文件 `client_config.yaml` 示例：

```yaml
server:
  url: "http://127.0.0.1:8000"
  timeout: 30

display:
  theme: "auto"
  table_max_rows: 20
  show_progress: true

ui:
  use_rich: true
  use_cmd2: true
  emoji: true

logging:
  level: "INFO"
```

---

## 🔧 开发路线图

### v0.1.0（当前版本）

- ✅ 动画管理功能
- ✅ RSS 源管理功能
- ✅ 智能解析功能（蜜柑计划）
- ✅ 链接管理功能
- ✅ 下载器管理功能（Mock 实现）
- ✅ 下载任务管理功能（Mock 实现）
- ✅ 定时任务调度功能
- ✅ 模块化 API 路由和 Schemas 设计
- ✅ 完整的测试套件

### v0.2.0（下一版本）

- 🚧 RSS 源自动检查
- 🚧 链接自动下载
- 🚧 下载状态同步
- 🚧 命令行界面实现（rss、link、downloader、download、status 命令）

### v0.3.0+（未来计划）

- 📋 多下载器真实实现（aria2、pikpak、qBittorrent）
- 📋 用户认证和权限管理
- 📋 Web 界面
- 📋 移动端支持

---

## 🤝 贡献指南

我们欢迎任何形式的贡献！

### 如何贡献

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

### 代码规范

- 遵循 PEP 8 代码风格
- 添加适当的注释和文档字符串
- 为新功能添加测试
- 确保所有测试通过

### 报告问题

如果您发现了 bug 或有功能建议，请在 GitHub 上提交 Issue。

---

## 📄 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件

---

## 👥 联系方式

- 项目主页：https://github.com/insomniacdoll/animeloader
- 问题反馈：https://github.com/insomniacdoll/animeloader/issues

---

## 🙏 致谢

感谢所有为这个项目做出贡献的开发者！

---

<div align="center">

**Made with ❤️ by AnimeLoader Team**

</div>