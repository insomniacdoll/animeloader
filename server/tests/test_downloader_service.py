...
def test_downloader_service():
    ...
    try:
        ...
        assert default_downloader.id == downloader1.id
        ...
        assert aria2_downloader.id == downloader2.id
        ...
        assert downloader.name == "Mock下载器(已更新)"
        assert downloader.max_concurrent_tasks == 5
        print(f"✓ 更新下载器: {downloader.name}")
        
        # 测试8: 测试下载器连接
        result = downloader_service.test_downloader(downloader1.id)
        assert result["success"] is True
        print(f"✓ 测试下载器连接: {result['message']}")
        
        # 测试9: 获取下载器状态
        status = downloader_service.get_downloader_status(downloader1.id)
        assert status["success"] is True
        print(f"✓ 获取下载器状态: {status['name']}")
        
        # 测试10: 设置默认下载器
        downloader = downloader_service.set_default_downloader(downloader2.id)
        assert downloader.is_default is True
        print(f"✓ 设置默认下载器: {downloader.name}")
