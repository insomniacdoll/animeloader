...
class ExampleRSSParser(BaseRSSParser):
    """示例RSS解析器"""
    ...
    def _extract_download_links(self, entry: Any) -> List[Dict[str, Any]]:
        ...
        # 示例3: 从description中提取链接（使用正则表达式）
        description = entry.get('description', '') or entry.get('summary', '')
        import re
        
        # 提取磁力链接
        magnet_links = re.findall(r'magnet:\?xt=[^\s<>"\']+', description)
        for magnet_url in magnet_links:
            links.append({
                'link_type': 'magnet',
                'url': magnet_url,
                'file_size': 0,
                'filename': entry.get('title', ''),
                'meta_data': f'magnet_link:{magnet_url}'
            })
...
# 使用示例：
#
# 1. 创建自定义解析器类（继承自BaseRSSParser）
# 2. 实现 can_parse() 方法，判断是否可以解析该URL
# 3. 实现 parse_rss() 方法，解析RSS源
# 4. 实现 get_site_name() 方法，返回网站名称
# 5. 可选：重写 _extract_download_links() 方法，实现自定义的链接提取逻辑
#
# 6. 使用解析器：
#    from server.site_parsers.example_rss_parser import ExampleRSSParser
#
#    parser = ExampleRSSParser()
#    result = parser.parse_rss('https://example.com/rss')